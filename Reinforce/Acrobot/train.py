...
net = Policy(input_dims=input_dim, hidden_dim=hidden_dim,
...
def train(episodes = 5000, max_iter = 1000):
    ...
    for episode_idx in range(1, episodes + 1):
        ...
        if np.mean(scores_window) >= threshold:
            ...
            torch.save(data, f"Acrobot_reinforce_{episode_idx}_steps.pt")
            break
...
plt.xlabel("# Episodes")
plt.ylabel("Scores")
plt.savefig("Acrobot_reinforce_results.png")
plt.show()